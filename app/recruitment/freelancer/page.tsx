...
import { freelancerRecruitmentApi } from '../../../lib/api';
...
export default function FreelancerRecruitmentPage() {
  ...
  const loadApplications = async () => {
    try {
      const response = await freelancerRecruitmentApi.list();
      setApplications(Array.isArray(response) ? response : []);
    } catch (error) {
      console.error('Failed to load freelancer applications', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      const updated = await freelancerRecruitmentApi.update(id, { status: newStatus as ApplicationStatus });
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this freelancer application?')) return;
    try {
      await freelancerRecruitmentApi.remove(id);
      setApplications((prev) => prev.filter((app) => app.id !== id));
    } catch (error) {
      console.error('Failed to delete freelancer application', error);
